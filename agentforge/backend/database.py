...
# pxxl/Railway Postgres uses postgres:// prefix — fix for SQLAlchemy
...
class Session(Base):
    ...
    scratchpad  : Mapped[str | None] = mapped_column(Text)
    created_at  : Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
...
class ApiSettings(Base):
    ...
    github_client_id   : Mapped[str | None] = mapped_column(Text)
    github_client_secret: Mapped[str | None] = mapped_column(Text)
