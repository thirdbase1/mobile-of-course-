...
export default function MessageBubble({ msg }: MessageProps) {
  ...
  if (isToolCall || isToolResult) {
    return (
      <div className={clsx("my-1 max-w-3xl w-full px-4 ml-auto")}>
        <div
        ...
        >
          <div className="w-5 h-5 rounded bg-white/5 flex items-center justify-center border border-white/5 group-hover:border-primary/20">
            {isToolCall ? <Terminal className="w-2.5 h-2.5 text-primary" /> : <CheckCircle2 className="w-2.5 h-2.5 text-green-500" />}
          </div>
  ...
  return (
    <div className={clsx("py-6 px-4 w-full flex", isUser ? "justify-start" : "justify-end")}>
      <div className={clsx("max-w-2xl w-full flex flex-col gap-3", isUser ? "items-start" : "items-end text-right")}>
        <div className={clsx("flex items-center gap-2 mb-1", isUser ? "flex-row" : "flex-row-reverse")}>
           ...
           <span className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/80">{isUser ? 'Commander' : 'Executor'}</span>
           {!isUser && reasoningContent && (
                 <button onClick={() => setShowReasoning(!showReasoning)} className="flex items-center gap-1.5 px-2 py-0.5 rounded bg-white/5 text-[9px] font-bold text-muted-foreground hover:text-white transition-colors">
                   <Eye className="w-2.5 h-2.5" /> {showReasoning ? 'Hide Reasoning' : 'View Reasoning'}
                 </button>
           )}
        </div>
        <AnimatePresence>
          {showReasoning && reasoningContent && (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full p-4 bg-white/[0.02] border border-white/5 rounded-xl text-xs text-muted-foreground/60 leading-relaxed italic text-left">
                  {reasoningContent}
              </motion.div>
          )}
        </AnimatePresence>
        <div className={clsx("prose prose-invert prose-zinc prose-sm max-w-none leading-relaxed p-4 rounded-2xl shadow-2xl", isUser ? "bg-white/[0.03] border border-white/5 rounded-tl-none text-left" : "bg-primary/5 border border-primary/10 rounded-tr-none text-left")}>
